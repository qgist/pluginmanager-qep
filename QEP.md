...
One aspect that immediately becomes obvious when reading QGIS' Python code is the fact that there are absolutely unnecessary relics of an automatic `2to3` conversion all over the place. This tells a lot about the state of the code already. `2to3` is an extremely useful tool, but even under the best of circumstances, its result requires intense manual editing which appears to be missing. There are also remains of pure Python 2 code within the code base in actually critical places (e.g. the module import infrastructure), which were never properly removed although QGIS has not been able to build with Python 2 for many years.
