...
Most of the Python code cares very little about mutable Python data types. A few calls to `copy` methods in interesting places tell a story of running into issues related to mutability without caring too much about it. If a potential user relying on the correct use of mutability would use some of the Python APIs in `qgis.utils` and `qgis.pyplugin_installer`, it would directly cause layers of bugs.
